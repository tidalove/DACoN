...
class DACoNTinyModel(nn.Module):
    def __init__(self, dacon_config, version):
        super(DACoNTinyModel, self).__init__()

        self.version = version
        model_type = dacon_config["dino_model_type"]

        try:
            model = DINOv3Package.get_model('vitt16')
            model.load_state_dict(torch.load(f'/orcd/home/002/kav/distill/scripts/exps/{model_type}/exported_models/exported_last.pt', weights_only=True))
            self.dino = model

        except Exception as e:
            raise ImportError(
                f"Failed to load model '{model_type}'."
                f"Error: {e}"
            )
    ...
    def get_dino_feats_map(self, images):
        ...
        feat_W = self.dino_input_size[1] // 16
